...
function this:AcquireLock(missile)
	...
	if self.LastSeek + self.WireSeekDelay <= curTime then
		...
		if wireEnt then return wireEnt end --print("wiremod provided", wireEnt)
	end

	if self.LastSeek + self.SeekDelay > curTime then return nil end --print("tried seeking within timeout period")
	...
	if foundCt < 2 then return found[1] end --print("shortcircuited and found", found[1])
	local missilePos = missile:GetPos()
	local missileForward = missile:GetForward()
	...
	for k, ent in next, found, lastKey do
		...
		if currentDot > highestDot and self:HasLOSVisibility(currentEnt, missile) then
			...
			if currentDot >= self.SeekTolerance then return currentEnt end --print("found", mostCentralEnt, "in tolerance")
		end
	end
