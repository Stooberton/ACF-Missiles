...
local MinimumDistance = 196.85 * 196.85 -- Minimum distance for a target to be considered (5m)
...
local function GetWireTarget(GuidanceData, Missile)
	...
	if TargetOutput and IsValid(TargetOutput.Value) then
		...
		local IsMissile = ACF_ActiveMissiles[Target]
		...
		if IsMissile and HasLOS and TargetDot >= SeekConeCos then
			return Target
		end
	end
end

local function GetMissilesInCone(GuidanceData, Missile)
	...
	if next(Missiles) then
		...
		for Target in pairs(Missiles) do
			if IsValid(Target) and Target ~= Missile then
				local TargetDot, TargetPos = GetTargetData(Missile, Target)
				local TargetDist = TargetPos:LengthSqr()

				if TargetDist >= MinimumDistance and TargetDot > SeekConeCos then
					Result[Target] = TargetDot
				end
			end
		end
	end
...
local function AcquireLock(GuidanceData, Missile)
	...
	local Missiles = GetMissilesInCone(GuidanceData, Missile)
	if not next(Missiles) then return nil end

	local Ent, CurrentDot = next(Missiles)
	local HighestDot = -1
	...
	repeat
		if CurrentDot > HighestDot and HasLOSVisibility(Missile, Ent) then
			...
			if CurrentDot >= SeekTolerance then
				return Ent
			end
		end

		Ent, CurrentDot = next(Missiles, Ent)
	until not Ent
...
function Guidance:Init()
	...
	self.SeekDelay = 0.2 -- This instance must wait this long between target seeks.
