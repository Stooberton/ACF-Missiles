...
local pagelistRegex = '<div id="wiki%-content">' .. '.*<table class="files">' .. '(.-)' .. '</table>' -- div id most closely containing the page list -- containing div class -- the list within -- class-closing div
local linklistRegexGlobal = '<a href="(.-)">' -- get partial url in the listed links
local extractPagenameRegex = '/.-/.-/wiki/(.*)'
...
function GitRC.GetWikiPages(name, repo, callback)
	http.Fetch(string.format(pagelist, name, repo), function(html)
		wikiPagesSuccess(html, callback)
	end, function()
		wikiPagesFailed(callback)
	end)
end
