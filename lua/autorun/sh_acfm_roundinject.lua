...
local function checkIfDataIsMissile(data)
	...
	if not (class and class.gunclass) then return oldDisplayData(data) end
	...
	for k, v in pairs(roundTypes) do
		...
		if oldDisplayData then
			v.getDisplayData = function(data)
				if not checkIfDataIsMissile(data) then return oldDisplayData(data) end
	...
	for k, v in pairs(roundTypes) do
		...
		if oldCratetxt then
			v.cratetxt = function(data, crate)
				...
				if not checkIfDataIsMissile(data) then return origCrateTxt end
